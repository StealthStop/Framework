...
BTagEntry::Parameters::Parameters(OperatingPoint op, std::string measurement_type, std::string sys_type, JetFlavor jf,
                                  float eta_min, float eta_max, float pt_min, float pt_max, float discr_min, float discr_max)
    : operatingPoint(op), measurementType(measurement_type), sysType(sys_type), jetFlavor(jf)
    , etaMin(eta_min), etaMax(eta_max), ptMin(pt_min), ptMax(pt_max), discrMin(discr_min), discrMax(discr_max)
{
...
BTagEntry::BTagEntry(const std::string& csvLine)
{
    ...
    if(f1.IsZombie()) 
    {
        std::cerr << "ERROR in BTagCalibration: Invalid csv line; formula does not compile: " << csvLine << std::endl;
        throw std::exception();
    }
    ...
    params = BTagEntry::Parameters(BTagEntry::OperatingPoint(op), vec[1], vec[2], BTagEntry::JetFlavor(jf),
                                   stof(vec[4]), stof(vec[5]), stof(vec[6]), stof(vec[7]), stof(vec[8]), stof(vec[9]));
}
    ...
    , params(p)
{
    ...
    if (f1.IsZombie()) 
    {
        std::cerr << "ERROR in BTagCalibration: Invalid func string; formula does not compile: " << func << std::endl;;
        throw std::exception();
    }
}
    ...
    , params(p)
{
    if(func->IsZombie()) 
    {
        std::cerr << "ERROR in BTagCalibration: Invalid TF1 function; function is zombie: " << func->GetName();
        throw std::exception();
    }
}
...
std::string th1ToFormulaLin(const TH1* hist) 
{
    ...
    {
        char tmp_buff[50];
        sprintf(tmp_buff, "x<%g ? %g : ", axis->GetBinUpEdge(i), hist->GetBinContent(i));
        buff << tmp_buff;
    }
    buff << 0.0;  // default value
    return buff.str();
}
    ...
    : params(p)
{
    ...
    if(f1.IsZombie()) 
    {
        std::cerr << "ERROR in BTagCalibration: Invalid histogram; formula does not compile (>150 bins?): " << hist->GetName() << std::endl;
        throw std::exception();
    }
}
...
BTagCalibration::BTagCalibration(const std::string& taggr, const std::string& filename)
    : tagger_(taggr)
{
    std::ifstream ifs(filename);
    if(!ifs.good()) 
    {
        std::cerr << "ERROR in BTagCalibration: input file not available: " << filename << std::endl;
        throw std::exception();
    }
    readCSV(ifs);
    ifs.close();
...
const std::vector<BTagEntry>& BTagCalibration::getEntries(const BTagEntry::Parameters& par) const
{
    ...
    if (!data_.count(tok)) {
        std::cerr << "ERROR in BTagCalibration: (OperatingPoint, measurementType, sysType) not available: " << tok << std::endl;
        throw std::exception();
    }
    return data_.at(tok);
}
...
BTagCalibrationReader::BTagCalibrationReaderImpl::TmpEntry::TmpEntry(float etaMin, float etaMax, float ptMin, float ptMax, float discrMin, float discrMax, TF1 func)
    : etaMin(etaMin), etaMax(etaMax), ptMin(ptMin), ptMax(ptMax), discrMin(discrMin), discrMax(discrMax), func(func)
{
}
    ...
    , useAbsEta_(3, true)
{
    for(const std::string& ost : otherSysTypes) 
    {
        ...
        {
            std::cerr << "ERROR in BTagCalibration: Every otherSysType should only be given once. Duplicate: " << ost << std::endl;
            throw std::exception();
        }
...
void BTagCalibrationReader::BTagCalibrationReaderImpl::load(const BTagCalibration& c, BTagEntry::JetFlavor jf, std::string measurementType)
{
    if(tmpData_[jf].size()) 
    {
        std::cerr << "ERROR in BTagCalibration: Data for this jet-flavor is already loaded: " << jf << std::endl;
        throw std::exception();
    }
    ...
    for(unsigned ibe = 0; ibe < entries.size(); ++ibe) 
    {
        ...
        TmpEntry te(be.params.etaMin, be.params.etaMax, be.params.ptMin, be.params.ptMax, be.params.discrMin, be.params.discrMax);
...
float BTagCalibrationReader::BTagCalibrationReaderImpl::eval(BTagEntry::JetFlavor jf, float eta, float pt, float discr) const
{
...
float BTagCalibrationReader::BTagCalibrationReaderImpl::eval_auto_bounds(const std::string& sys, BTagEntry::JetFlavor jf, float eta, float pt, float discr) const
{
    ...
    {
        std::cerr << "ERROR in BTagCalibration: sysType not available (maybe not loaded?): " << sys << std::endl;
        throw std::exception();
    }
...
std::pair<float, float> BTagCalibrationReader::BTagCalibrationReaderImpl::min_max_pt(BTagEntry::JetFlavor jf, float eta, float discr) const
{
    bool use_discr = (op_ == BTagEntry::OP_RESHAPING);
    if(useAbsEta_[jf] && eta < 0) 
    {
...
BTagCalibrationReader::BTagCalibrationReader(BTagEntry::OperatingPoint op, const std::string & sysType, const std::vector<std::string>& otherSysTypes)
    : pimpl(new BTagCalibrationReaderImpl(op, sysType, otherSysTypes)) 
{
}

void BTagCalibrationReader::load(const BTagCalibration& c, BTagEntry::JetFlavor jf, const std::string& measurementType)
{
    pimpl->load(c, jf, measurementType);
}

float BTagCalibrationReader::eval(BTagEntry::JetFlavor jf, float eta, float pt, float discr) const
{
    return pimpl->eval(jf, eta, pt, discr);
}

float BTagCalibrationReader::eval_auto_bounds(const std::string& sys, BTagEntry::JetFlavor jf, float eta, float pt, float discr) const
{
