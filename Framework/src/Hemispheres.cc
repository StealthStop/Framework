...
Hemisphere::Hemisphere(vector<float> Px_vector, vector<float> Py_vector, vector<float> Pz_vector, vector<float> E_vector) : 
                       ...
                       seed_meth(NoSeed), hemi_meth(0), status(0), dRminSeed1(0.5), nItermax(100), 
                       rejectISR(0), rejectISRPt(0), rejectISRPtmax(10000.), rejectISRDR(0), rejectISRDRmax(100.), dbg(0)  
{	
    for(int i = 0; i < (int) Object_Px.size(); i++)
    {
...
// NewAxis1_xxx, NewAxis1_xxx : temporary axes for calculation in association methods 2 and 3
    ...
    if (seed_meth == PtTimesDeltaRSeed) 
    {
        ...
        {
            Object_Group[i] = 0;
            //cout << "Object_Px[i] = " << Object_Px[i] << ", Object_Py[i] = " << Object_Py[i] << ", Object_Pz[i] = " << Object_Pz[i] << "  << endl;
    ...
    // iterate to associate all objects to hemispheres (methods 1 to 3 only) until no objects are moved from one to the other hemisphere or the maximum number of iterations is reached
...
{
    // tries to avoid including ISR jets into hemispheres, iterate to remove all ISR objects from the hemispheres until no ISR objects are found
